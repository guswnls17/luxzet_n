...
<%	 
	...
	SQLST = " UPDATE boardq SET read_num=read_num + 1 WHERE idx="&idx
	Conn.Execute(SQLST)

	Set rs = Server.createObject("ADODB.RecordSet")
	sql = "select * from boardq where idx = "&idx
	rs.Open sql, Conn, 1	
	...
	sql = "select * from boardq where idx = '"&question_idx&"'"
	rs.Open sql, Conn, 1
	if not rs.eof then
	...
	if secret_flag = "1" and write_id <> session("member") and question_write_id <> session("member") And session("member") <> "0000000000" then 
		if instr(Request.ServerVariables("HTTP_REFERER"),"login.asp") > 0 then
			call alertGo("You do not have read permissions.","support.asp")
		else
     	...
     	<colgroup>
          ...
          <tr>
            ...
            <tr>
              <th>Attachments</th>
              <td>
				<a href="/_uploadq/<%=filename%>" class="community_bar_unlink_text">&nbsp;<%=filename%></a>&nbsp;
			  </td>
            </tr>
            <tr>
              <td colspan="2">
			...
			<tr>
              <td colspan="2">
                   <table width="100%" border="0" cellspacing="0" cellpadding="0" class="btn_center">
					<tr> 
					<%if session("member") = "0000000000" or session("member") = write_id then %>	
							  <td><div align="center"><a href="support.asp?code=<%=code%>&mode=write&idx=<%=idx%>&page=<%=page%>&searchField=<%=searchField%>&searchValue=<%=searchValue%>"  onfocus="this.blur()" class="btn_style02"><span>Modify</span></a></div></td>
							  <td>		
								<form name="form" method="post" action="./customer/cqna_save.asp" enctype="multipart/form-data">
					  ...
					  <td align="center">
					  <div align="center"><a href="support.asp?code=<%=code%>&mode=list&page=<%=page%>&searchField=<%=searchField%>&searchValue=<%=searchValue%>"  onfocus="this.blur()" class="btn_style01"><span>List</span></a></div> </td>
					</tr>
				  </table>
              </td>
            </tr>
