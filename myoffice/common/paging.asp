...
'	Response.write "startpage(" & startpage & ")"
	If pagecount >= startpage + 10 Then endpage = startpage + 10 Else endpage = pagecount + 1
'	Response.write "endpage(" & endpage & ")"
%>
				<div class="paging">
<% If group > 0 then%>
					<a href="<%=linkurl%>&page=1"><img src="/images/board_paging_start.gif" alt="마지막페이지" /></a>
					<a href="<%=linkurl%>&page=<%=(group - 1) * 10 + 1%>"><img src="/images/board_paging_prev.gif" alt="다음페이지" /></a>
<% Else %>
...
<% If pagecount > 10 And (Group + 1) * 10 < pagecount Then%>
					<a href="<%=linkurl%>&page=<%=(Group + 1) * 10 + 1%>"><img src="/images/board_paging_next.gif" alt="이전페이지" /></a>
					<a href="<%=linkurl%>&page=<%=pagecount%>"><img src="/images/board_paging_end.gif" alt="처음페이지" /></a>
<% Else %>
