...
<% If session("userid") = "ADMIN" Then %> 
		  <tr> 
			<td height="40" colspan='4'><div align="center"><a href="notice.asp?mode=write&code=<%=code%>"  onfocus="this.blur()" class="btn_style02" ><span>Writing</span></a></div>
...
Function Get_viewPage(byRef conn,write_id,secret_flag,idx,question_idx)   
	...
	if session("userid") <> "" then
		...
		if (secret_flag = "1" and write_id <> session("userid") and question_write_id <> session("userid") And session("userid") <> "ADMIN") then 
			...
			Get_viewPage = "notice.asp?idx=" & idx & "&code="&code&"&mode=view&page=" & page & "&searchField=" & searchField & "&searchValue=" & searchValue
		end if	
	else
