...
<%
    ...
    fileSaveDir = server.mappath("../") & "\_uploadq\" 'upload 패스
	abDir = "/_uploadq/"
...
'		if session("userid") <> write_id and session("userid") <> "ADMIN" then call alertBack("NO! Modify!!")
		...
		if(not isnull(getfile)) then
			...
			timestamp = DateDiff("s", 1970-1-1, now()) 		' timiestamp 값을 앞에 넣어 준다
			
			If getfile.FileExists Then '업로드된 파일이 있으면
				name1 = timestamp&"_"&getfile.RawFileName '파일이름 설정 후
...
'				response.write fileSaveDir&name1
		...
		sql = " UPDATE boardq SET  subject=N'"&subject&"', content=N'"&content&"' ,secret_flag='"&secret_flag&"' " 
...
'		response.write sql
		If f_sql_modify(db_conn, sql) < 0 Then
			...
			response.write "<script>window.location.href='../support.asp?code="& code &"'</script>"  
		End If
		
		response.write "<script>window.location.href='../support.asp?code="&code&"&idx="&idx&"&page="&page&"&searchField="&searchField&"&searchValue="&searchValue&"';</script>" 
...
'		if (session("userid") <> write_id and session("userid") <> "ADMIN") then call alertBack("NO! Modify!!")
		...
		sql = " DELETE From boardq WHERE code='"& code &"' and idx="&idx
		If f_sql_modify(db_conn, sql) < 0 Then
			jsMessageBox language(lan,174), "", "document"
			response.write "<script>window.location.href='../support.asp?code="& code &"'</script>"  
		End If
	...
	if  flag = "insert" then	
		...
		sq1 = "select max(step) as st from boardq where code='"& code &"'" 
		...
		if(not isnull(getfile1)) then
			...
			timestampi = DateDiff("s", 1970-1-1, now()) 		' timiestamp 값을 앞에 넣어 준다
			
			If getfile1.FileExists Then '업로드된 파일이 있으면
				name1 = timestampi&"_"&getfile1.RawFileName '파일이름 설정 후
...
'				response.write fileSaveDir&name1
		...
		sql = "INSERT INTO boardq (write_id,write_name,subject,content,filename,step,depth,question_idx,secret_flag,code) VALUES  " 
		sql = sql & " ('"& session("member") &"','"& session("username") &"',N'"&subject&"',N'"&content&"','"&filename&"','"&step_id&"',"&depth&","&question_idx&",'"&secret_flag&"','"&code&"') "
