...
'	Verification = ""
	...
	If Trim(request("delivery")) = "1" Then 'Gcoin-1,현금-0 구분 
		'결제금액+8%차감금--삭제
		'coin_amount=CLng(n_amount+(n_amount*0.08))
		coin_amount=CLng(account)

		sq1 = "select Isnull(mem_ye_kum,0), isnull(mem_up_jum, 0) from mmember_temp where mem_code = '" & memno & "' "
			If f_sql_select(db_conn, sq1, arrData) > 0 Then
				emoney = CLng(arrData(0, 0)) '보유 이머니 
				iomoney = CLng(arrData(1, 0)) '보유 iowallet 
			End If
			
			if Verification = "" then 
				If coin_amount > (emoney) Then
					jsMessageBox "Payment amount is greater than Gcoin.", "", "docume" 	
					Response.end
				End If
			end if
	 End If
	...
	Function hoo_Cntchk()
		...
		If arrData(0, 0) >= 2 Then
			...
			Response.write "<script>alert('Placement check.'); history.back(); </script>"
			Response.end
	...
	For i = 0 To prod_cnt - 1
		...
		If prodinfo <> "" Then
			...
			if coin_ewallet > 0 then 
				...
				& "'" & (CLng(qty) * CLng(arrProd(3))) & "', '"&sale_etc1&"', 0, '" & sale_yekum & "', 0, 0, 0, '"&sale_gubun&"', '"&Trim(request("sale_sw"))&"', '"&Trim(request("delivery"))&"', " _
...
Call f_sql_modify(db_conn,sql)
	...
	Next
	 ...
	 if coin_ewallet > 0 then 
		...
		& "'" & n_point & "', '"&sale_etc1_s01&"', 0, '" & sale_yekum_s01 & "', 0, 0, 0, '"&sale_gubun&"', 0, 0, 0, " _
	...
	If Trim(request("delivery")) = "1" Then 'Gcoin-1,현금-0 구분 
		...
		sq1 = "select Isnull(mem_ye_kum,0), isnull(mem_up_jum, 0) from mmember where mem_code = '" & memno & "' "
			If f_sql_select(db_conn, sq1, arrData) > 0 Then
				emoney = CLng(arrData(0, 0)) '보유 이머니 
				iomoney = CLng(arrData(1, 0)) '보유 이머니 
			End If
		...
		sql = ""	
		sql = sql & "update mmember set mem_ye_kum = mem_ye_kum - " & coin_ewallet & " , mem_up_jum = mem_up_jum - " & coin_iowallet & "  "_
...
'			End If
		...
		if coin_iowallet > 0 then 
			sql = ""	
			sql = sql & "insert into mmember_iowallet (iowallet_mem_code, iowallet_idate, iowallet_no, " _
			& "iowallet_mem_name, iowallet_kum, iowallet_istate, iowallet_ostate, iowallet_give_code) values  (" _
			& "'" & ps_mem_code & "', convert(varchar(8), getdate(), 112), " _
			& "isnull((select isnull(max(iowallet_no), 0) from mmember_iowallet b " _
			& "where '"&ps_mem_code&"' = b.iowallet_mem_code and b.iowallet_idate = convert(varchar(8), getdate(), 112)), 0) + 1, " _
			& "'"&ps_mem_name&"', floor(" & CLng(coin_iowallet) & "), " _
			& "'2', '2','' )" 
		
			If f_sql_modify(db_conn, sql) < 0 Then
				jsMessageBox "An error has occurred in the storage process5. Contact your administrator." + sql, "", "docume"
			End If 
