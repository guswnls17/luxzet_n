...
ecpprice   = TRIM(request("ecp")) 

price_usd   = TRIM(request("price_usd"))    
lxzusd   = TRIM(request("lxzusd")) 

lxzusdt  = round((Cdbl(price_usd)/Cdbl(lxzusd)),8)

atype = TRIM(request("type"))               
from_address     = TRIM(request("from_address"))
amount   = round(Cdbl(TRIM(request("amunt"))),4)
...
If chash <> "" Then
	...
	sql = "select * from lxzcoin where lxz_hash = '"&chash&"'" 
'	Response.write sql
	If f_sql_select(db_conn, sql, arrData) = 0 Then
		...
		sqli = "insert into lxzcoin (price_usd, lxz_hash, lxz_type, lxz_amount, lxz_fromaddr, idate, lxz_exchange ) values ('"&price_usd&"','"&chash&"','"&atype&"','"&amount&"','"&from_address&"', '"&idate&"', '"&lxzusdt&"') "
		
		If f_sql_modify(db_conn, sqli) < 0 Then
		End If
		
		sqlb = "select erc_seq, erc_amount, erc_hoocode, bit_sale_number, erc_memcode, erc_memid from purchase_erc2 where erc_iaddr = '"&from_address&"' and erc_amount <= '" &amount& "' and erc_odate >= '" & odate & "' and erc_check = 0 order by erc_seq "
		bseq = ""
		bhoocode = ""
		bsalenumber = ""
		lxzcoin = ""
...
'		Response.write sqlb
		If f_sql_select(db_conn, sqlb, arrDatab) > 0 Then						'purchase_erc 해당 정보가 있을 경우 
			...
			lxzcoin = arrDatab(1,0)
			mem_code = arrDatab(4,0)
			mem_id = arrDatab(5,0)
			
			lxzcoin = Cdbl(lxzcoin)
			amount = Cdbl(amount)
...
'						jsMessageBox "An error has occurred in the storage process. Contact your administrator num1.", "", "document" '저장 처리 중 오류가 발생하였습니다. 관리자에 문의하세요
				sqlu = "update lxzcoin set lxz_bigo = 'update fail' where lxz_hash = '"&chash&"'  "
...
'						jsMessageBox "An error has occurred in the storage process. Contact your administrator num1.", "", "document" '저장 처리 중 오류가 발생하였습니다. 관리자에 문의하세요
				Else
					...
					sqlb = "update purchase_erc2 set erc_check = '1' , erc_txid = '" & chash & "' , new_sale_number = '" & orderNumber & "' where erc_seq = '" & bseq & "' "
					If f_sql_modify(db_conn, sqlb) < 0 Then
					
					End If 
					
					ecpamount = round((Cdbl(amount)*Cdbl(price_usd))/Cdbl(ecpprice),8)
					
					lxzcoin_1 = lxzcoin * 1.2
			...
			salecoin = amount - lxzcoin
			
			ppskum = round(salecoin*lxzusdt,2)
