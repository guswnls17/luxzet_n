...
<%
	...
	linkurl = "coin.asp?a=1"

    sql = "select mem_up_jum, isNull(mem_ye_kum,0) " _
		& "from mmember " _
...
function input_check(){
    ...
    if (parseInt(frm.per_amount.value) > parseInt('<%=emoney%>')) {
      ...
      <div class="col-lg-12">
        <div class="card p-4">
            <div class="card-search-body">
            	<form name="frmMain" method="post" action="./emoney/emoney_proc.asp" onsubmit="return input_check();" target='hiddenprocess'>
<input type='hidden' name='command'>
                	...
                	<tbody><tr>
                	  <th class="border-0"><%=language(lan,141)%></th>
                	  <td class="border-0">$<%If emoney <> "" Then %><%=FormatNumber(emoney, 2)%><%Else%>0<% End If%></td>
                	</tr>
                	<tr>
...
<!--						<option value='0'>Point conversion</option>  
							<option value='1'><%=language(lan,139)%></option> -->
	<!--						<option value='3'>Gift giving</option>   
							<option value='5'><%=language(lan,273)%></option>
							<option value='8'><%=language(lan,146)%></option>  -->
							<option value='9'><%=language(lan,263)%></option>
<!--							<option value='A'><%=language(lan,274)%></option>   -->
                        </select>
                      </div>
	                  </td>
                	</tr>
                	  ...
                	  <td>
                      	<div class="form-group">
                            <div class="input-group">
								<input type="text" name='n_amount' id='i_amount' value='' maxlength='10' class="form-control col-md-4" onKeyUp="use_payment();">
      ...
      <div class="col-lg-12">
        <div class="card">
          <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-lg text-uppercase"><%=language(lan,52)%></h6>
          </div>
...
<%
    ...
    sql = "select emoney_idate, emoney_no, " _
		& "case emoney_istate when '1' then 'Cash conversion application' when '2' then 'Sales Registration' " _
		& "when '0' then 'Point conversion' when '4' then 'E-Wallet purchase request'  when '5' then 'BITCOIN' when '6' then 'Coin sales' when '7' then 'Cash deposit' when '8' then 'Ethereum' when '9' then 'LXZ' when 'A' then 'ECP+' else '' end, " _
		& "case emoney_ostate when '1' then 'Payments' when '2' then 'Wait' when '9' then 'Cancel' when '0' then 'Unpaid' else '' end, " _
		& "isnull((select mem_name from mmember_temp where mem_code = emoney_give_code), ''), " _
		& "emoney_odate, isnull(emoney_kum,0) from mmember_emoney " _
		& "where emoney_mem_code = '" & session("member") & "' " _
		& " and  emoney_idate > '20161113' " _
		& "order by emoney_idate , emoney_no "
                ...
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                  <thead>
                    <tr>
						...
						<th><%=language(lan,147)%></th>
						<th><%=language(lan,148)%></th>
						<th><%=language(lan,149)%></th>
						<th><%=language(lan,150)%></th>
						<th><%=language(lan,151)%></th>
						<th><%=language(lan,152)%></th>
						<th><%=language(lan,153)%></th>
                    </tr>
                  </thead>
                   <tbody>
<%
...
%>				  
              <tr>
                ...
                <td><%=arrData(4, i)%></td>
                <td><%=arrData(5, i)%></td>
                <td><%=FormatNumber(arrData(6, i), 2)%></td>
              </tr>
<%	Next    %>
                  </tbody>
                </table>
