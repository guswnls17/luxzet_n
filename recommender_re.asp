...
function select_result(num,uid,uname,agency,count){
	if(count > 1 ){
...
If scarch_name = "" Then
		sql = "select  mem_code, mem_id, mem_name, mem_jumin , (select count(mem_code) from mmember where mem_hoo_code = A.mem_code) "
		sql = sql & "from mmember A "
		sql = sql & "where 1=2 "
else
		sql = "select  mem_code, mem_id, mem_name, mem_jumin , (select count(mem_code) from mmember where mem_hoo_code = A.mem_code) "
		sql = sql & "from mmember A "
		sql = sql & "where mem_name like N'%" & scarch_name & "%' or mem_id =  '"& scarch_name & "'"
End If
  	...
  	<div class="row">
    	<div class="col-lg-12">
            ...
            <div class="cardsearch">
            	<form name='frmMain' method='post' action='recommender_re.asp'>
