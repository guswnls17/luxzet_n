...
If (request("command") = "save") then
	...
	sq1 = "select mem_acoin  from mmember where mem_code = '" & memno & "' "
	If f_sql_select(db_conn, sq1, arrData) > 0 Then
			vmoney = CLng(arrData(0, 0)) '보유 이머니 
	End If
		
	If n_amount > vmoney Then
		jsMessageBox "Payment amount is greater than LXZ coin.", "", "document" 	
	End If
    ...
    sql = "update mmember set mem_acoin = mem_acoin - " & totalAmount & " " _
		& "where mem_code = '" & memno & "' " _
		& "and '" & n_istate & "' in ('1', '2', '3','5','9') " _
		& "insert into coin_banking_his2 (mem_code, date, mem_name, seq, " _
		& "pay, gbn, state, bigo) " _
		& "select mem_code, convert(varchar(8), getdate(), 112), mem_name, " _
		& "isnull((select isnull(max(seq), 0) from coin_banking_his2 b " _
...
'	response.write sql
	...
	End If
		...
		jsMessageBox "The data has been saved.", "../twicecoin2.asp", "parent"
