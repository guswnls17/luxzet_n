...
//		echo "btcwon : ".$btcwon."<br/>";
	...
	if ($fp = fopen('https://v2data.coin2x.net/ticker_ecp', 'r')) {
	   $content = '';
	   // 전부 읽을때까지 계속 읽음
	   while ($line = fread($fp, 1024)) {
		  $content .= $line;
...
//		 echo "ecp price : ". $content;
	   ...
	   // 파일 open시 에러 발생
        ...
        $URL = 'http://www.chainvit.net/saveLXZ.asp';
        $ch = curl_init();
        curl_setopt ($ch, CURLOPT_URL, $URL);
        curl_setopt ($ch, CURLOPT_SSL_VERIFYPEER, 1);
        curl_setopt ($ch, CURLOPT_POST, 1);
        curl_setopt ($ch, CURLOPT_POSTFIELDS, 'price_usd='.$price_usd.'&type='.$type.'&from_address='.$from_address.'&amunt='.$amunt.'&hash='.$hash.'&lxzusd='.$usd.'&ecp='.$ecpprice);
