...
If (request("command") = "save") then
	...
	If totalAmount > mem_rewards Then
	...
	If n_istate = "3" Then
		ps_mem_code = ""
		ps_mem_name = ""
		sq1 = "select mem_code, mem_name from mmember where mem_code = '" & n_memno & "' "
		If f_sql_select(db_conn, sq1, arrData) > 0 Then
			...
			& "and '" & n_istate & "' in ('3') "
			If f_sql_modify(db_conn, sql) < 0 Then
				jsMessageBox "An error has occurred in the storage process7. Contact your administrator." + sql, "", "document"
			End If 
			...
			If f_sql_modify(db_conn, sql) < 0 Then
				jsMessageBox "An error has occurred in the storage process8. Contact your administrator." + sql, "", "document"
			End If 

		Else 
			sq1 = "select mem_code, mem_name from mmember_temp where mem_code = '" & n_memno & "' "
			If f_sql_select(db_conn, sq1, arrData) > 0 Then
				ps_mem_code = arrData(0, 0) '보유 이머니 
				...
				& "and '" & n_istate & "' in ('3') "
				If f_sql_modify(db_conn, sql) < 0 Then
					jsMessageBox "An error has occurred in the storage process9. Contact your administrator." + sql, "", "document"
				End If 
				...
				If f_sql_modify(db_conn, sql) < 0 Then
					jsMessageBox "An error has occurred in the storage process10. Contact your administrator." + sql, "", "document"
				End If 
			End If 
