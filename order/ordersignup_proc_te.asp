...
'	Verification   = TRIM(request("io_wallet"))
	...
	sql = "select mem_code from mmember where mem_code = '" & memno& "' "
		
	If f_sql_modify(db_conn, sql) < 0 Then
		jsMessageBox "Not saved.", "../purchase_re.asp", "parent"
	End If
	...
	if radio = "bitcoin" then    ' 비트코인 이면 
		...
		sql = "update mmember set mem_wbit_code = '" & bit_code & "' where mem_code = '" & memno& "' "
	...
	Call f_sql_modify(db_conn,sql)
		...
		& ")values('" &session("userid") & "','" & memno & "', '" & n_amount & "','" & bitAmt & "','" & bit_kum & "', '2', '','" & idate & "','" & odate & "','" & order_no & "','" & exchange_time & "') "
		
'		jsMessageBox sql,"","document"
	...
	elseif radio = "ecpcoin" then    ' ecp코인 이면 
	
		ecpAmt = request("ecpAmt")
		ecp_kum = request("ecp_kum")
		ecp_code = request("ecp_code")

		
		sql = "update purchase_erc set erc_check = '2' where erc_memcode = '" & memno& "' and erc_check = '0' "
	...
	Call f_sql_modify(db_conn,sql)
		...
		sq1 = "select comp_addr1 from ccompany  "

		compaddr = ""
		If f_sql_select(db_conn, sq1, arrData) > 0 Then
			compaddr = arrData(0, 0)
		End If 
		
'		compaddr = "testecpaddress"
		
		sql = "insert into purchase_erc ( erc_datetime,  erc_addr, erc_amount, erc_iaddr, erc_memcode, erc_memid, erc_check, erc_hoocode, erc_idate, erc_odate, bit_sale_number, erc_exchange, s_amount " _ 
		& ")values(getdate() ,'" & compaddr & "', '" & ecpAmt & "','" & ecp_code & "','" & memno & "', '" &session("userid") & "', '0', '','" & idate & "','" & odate & "','" & order_no & "','" & ecp_kum & "', '" & n_amount & "') "
		
		'jsMessageBox sql,"","document"
		
		If f_sql_modify(db_conn, sql) < 0 Then
			jsMessageBox "An error has occurred in the storage process-ecpcoin. Contact your administrator." , "", "document"
		End If
		
		jsMessageBox "The data has been saved.", "../order_ecp.asp", "parent"
	
	elseif radio = "lxzcoin" then    ' lxz코인 이면 
	
		lxzAmt = request("lxzAmt")
		lxz_kum = request("lxz_kum")
		lxz_code = request("lxz_code")

		
		sql = "update purchase_erc2 set erc_check = '2' where erc_memcode = '" & memno& "' and erc_check = '0' "
	...
	Call f_sql_modify(db_conn,sql)
		...
		sq1 = "select comp_addr2 from ccompany  "
...
'		compaddr = "testlxzaddress"
		
		sql = "insert into purchase_erc2 ( erc_datetime,  erc_addr, erc_amount, erc_iaddr, erc_memcode, erc_memid, erc_check, erc_hoocode, erc_idate, erc_odate, bit_sale_number, erc_exchange, s_amount " _ 
		& ")values(getdate() ,'" & compaddr & "', '" & lxzAmt & "','" & lxz_code & "','" & memno & "', '" &session("userid") & "', '0', '','" & idate & "','" & odate & "','" & order_no & "','" & lxz_kum & "', '" & n_amount & "') "
		
		'jsMessageBox sql,"","document"
		
		If f_sql_modify(db_conn, sql) < 0 Then
			jsMessageBox "An error has occurred in the storage process-lxzcoin. Contact your administrator." , "", "document"
		End If
		
		jsMessageBox "The data has been saved.", "../order_lxz.asp", "parent"
		...
		if Verification <> "" then
			...
			Else
				...
				response.write "window.location.replace('/myoffice/purchase.asp');"
		...
		if coin_ewallet > 0  then 
			r_charge = (CLng(coin_ewallet)*0.2)
		end if 
...
'					End If
	...
	'			End If
			...
			lxzproc = Cdbl(lxzAmt) * 1.2
			
			'프로시저 호출 
			sql = "exec coin_insert @ls_code='"&memno&"',@ls_date='"&sale_date&"',@ld_coin='"&lxzproc&"',@ls_chk='0'"
